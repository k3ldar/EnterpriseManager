...
namespace Heavenskincare.Website.Staff.Reports.Salon
{
    internal class PDFDailySalonReport : BasePDFReport
    {
        public PDFDailySalonReport(DateTime From, DateTime To, Therapists therapists)
            : base(UniqueFileName("DailySalonReport", true))
        {
        ...
        {
            Image jpg = Image.GetInstance(ImagePath + "SalonReportHeader.jpg");
            document.Add(jpg);
        ...
        private void LoadRefunds(Document document, DateTime From, DateTime To)
        {
            ...
            if (refunds.Count > 0)
            {
                ...
                foreach (Refund refund in refunds)
                {
                    ...
                    table.AddCell(new Phrase(refund.Date.ToString("dd/MM/yyyy hh:mm"), FontTextSmall));
