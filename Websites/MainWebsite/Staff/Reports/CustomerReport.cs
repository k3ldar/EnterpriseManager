...
namespace Heavenskincare.WebsiteTemplate.WebsiteReports
{
    public class CustomerReport : BasePDFReport
    {
        ...
        private decimal CheckPriceWithVAT(decimal value, double vatRate)
        {
            decimal Result = value;

            if (DALHelper.ShowVATInSubTotals)
            {
                Result += SharedUtils.VATCalculate(value, vatRate);
            }
        ...
        private void PrintInvoiceItems(Document document, Invoice inv)
        {
            ...
            foreach (InvoiceItem item in inv.InvoiceItems)
            {
                ...
                tblAddr.AddCell(NewCell(new Phrase(SharedUtils.FormatMoney(CheckPriceWithVAT(item.Cost, inv.VATRate), inv.Currency), FontText), 1, false, 2));
                tblAddr.AddCell(NewCell(new Phrase(item.Quantity.ToString(), FontText), 1, false, 2));
                tblAddr.AddCell(NewCell(new Phrase(SharedUtils.FormatMoney(CheckPriceWithVAT(item.Price, inv.VATRate), inv.Currency), FontText), 1, false, 2));
            }

            tblAddr.AddCell(NewCell(new Phrase("Subtotal", FontText), 5, false, 2));
            tblAddr.AddCell(NewCell(new Phrase(SharedUtils.FormatMoney(CheckPriceWithVAT(inv.InvoiceItems.SubTotal, inv.VATRate), inv.Currency), FontText), 1, false, 2));
