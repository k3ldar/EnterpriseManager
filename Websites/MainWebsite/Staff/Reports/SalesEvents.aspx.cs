...
namespace Heavenskincare.WebsiteTemplate.Reports
{
        ...
        void _stats_OnTimeLineEvent(object sender, Library.BOL.Statistics.Statistics.TimeLineEventArgs e)
        {
            ...
            if (cbShowTextAsHint.Checked)
                hint = String.Format("{0}\r\nDate: {6}\r\n{5}\r\nCost: {1}\r\nSales: {2}\r\nVisits: {3}\r\nCountry: {4}",
                    e.EventDescription, SharedUtils.FormatMoney(e.EventCost, GetWebsiteCurrency()), SharedUtils.FormatMoney(e.EventSales, GetWebsiteCurrency()), 
                    e.EventVisits, Countries.Get(e.EventCountry).Name, e.EventText, e.Date.ToShortDateString());

            switch (lstRange.SelectedValue)
            {
                case "Daily":
                    web.Library.Classes.Annotation.AddAnnotation(Chart1, _dataTable, e.Date, e.EventDescription, hint, cbShowRedLine.Checked, false, false, !cbShowTextAsHint.Checked);
                    break;

                case "Monthly":
                    web.Library.Classes.Annotation.AddAnnotation(Chart1, _dataTable, e.Date, e.EventDescription, hint, cbShowRedLine.Checked, true, false, !cbShowTextAsHint.Checked);
                    break;
            }
        }
