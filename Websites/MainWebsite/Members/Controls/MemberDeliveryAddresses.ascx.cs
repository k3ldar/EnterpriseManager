...
namespace Heavenskincare.WebsiteTemplate.Members.Controls
{
    public partial class MemberDeliveryAddresses : BaseControlClass
    {
        ...
        public void Refresh(DeliveryAddress address, Enums.Mode mode)
        {
            ...
            decimal shippingCosts = _deliveryAddress.Country.ShippingCosts;

            if (_deliveryAddress.Country.ID != GetUser().Country.ID)
                shippingCosts = shippingCosts * GetWebsiteCurrency().ConversionRate;

            pPostageCost.InnerHtml = String.Format("{1} {0}",
                SharedUtils.FormatMoney(shippingCosts, GetWebsiteCurrency(), false),
