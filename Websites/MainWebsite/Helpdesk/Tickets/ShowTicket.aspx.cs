...
namespace Heavenskincare.WebsiteTemplate.Helpdesk.Tickets
{
    public partial class ShowTicket : BaseWebForm
    {
        ...
        {
            return (Shared.Utilities.DateTimeToStr(_ticket.LastUpdated, WebCulture));
        }
        ...
        protected void GetTicketPosts()
        {
            foreach (SupportTicketMessage message in _ticket.Messages)
            {
                ...
                Heavenskincare.Helpdesk.Controls.HelpdeskTicketPost tp = (Heavenskincare.Helpdesk.Controls.HelpdeskTicketPost)LoadControl("~/Helpdesk/Controls/HelpdeskTicketPost.ascx");
                tp.Refresh(message);
                c.Controls.Add(tp);
            }
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string Name = HSCUtils.ReplaceHTMLElements(txtReplyName.Text).Trim();
                string Message = HSCUtils.ReplaceHTMLElements(txtReply.Text).Trim();
