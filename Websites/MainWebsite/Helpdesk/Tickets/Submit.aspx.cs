    ...
    public partial class Submit : BaseWebForm
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (VerificationImage1.IsValid)
            {
                try
                {
                    string Subject = HSCUtils.ReplaceHTMLElements(txtSubject.Text);
                    string Content = HSCUtils.ReplaceHTMLElements(txtMessage.Text);
                    string UserName = HSCUtils.ReplaceHTMLElements(txtName.Text);
                    string Email = HSCUtils.ReplaceHTMLElements(txtEmail.Text);

                    //validate the email address
                    if (!Shared.Utilities.IsValidEmail(Email))
                        throw new Exception(Languages.LanguageStrings.InvalidEmailAddress);

                    string TicketID = Shared.Utilities.GetRandomKey();
                    int Dept = Shared.Utilities.StrToInt(lstDepartment.SelectedValue, 0);
                    int Status = 1;
                    int Priority = Shared.Utilities.StrToInt(lstPriority.SelectedValue, 0);
                    ...
                    string EMail = String.Format(@"{0},<br /><br />Your ticket has been received, one of the staff members will review it and reply accordingly. " +
                        ...
                        "http://helpdesk.heavenskincare.com/Tickets/Showticket.aspx?TicketID={1}&Email={5}<br /><br />kind regards,<br /><br />Heaven Health & Beauty",
