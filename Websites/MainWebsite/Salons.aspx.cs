...
namespace Heavenskincare.WebsiteTemplate
{
    public partial class PageSalons : BaseWebForm
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            frmSalonFinder.Visible = Global.ShowSalonFinder;
            headerSalons.Visible = Global.ShowSalonFinder;

            btnFindSalon.Text = Languages.LanguageStrings.Search;
            headerClients.Visible = Global.ShowClientHeader;
            headerSalons.Visible = Global.ShowSalonHeader;
        }

        protected string BuildSalonList()
        {
            string Result = "";
            Salon salonID = Salons.Get(GetFormValue("ID", -1));
        ...
        private string FindNearestSalons()
        {
            ...
            string Postcode = HSCUtils.ReplaceHTMLElements(txtPostCode.Text, CaseType.Upper);
        ...
        protected override string BuildPagination(int PageCount, int ItemsPerPage, int CurrentPage, string Page)
        {
            string Result = "";
            Salon salonID = Salons.Get(GetFormValue("ID", -1));
            ...
            if (!IsPostBack)
            {
                Result = BuildPagination(PageCount, ItemsPerPage, CurrentPage, Page, "");
            }
