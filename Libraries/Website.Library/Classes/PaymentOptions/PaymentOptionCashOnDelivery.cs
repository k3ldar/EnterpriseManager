...
namespace Website.Library.Classes.PaymentOptions
{
    public sealed class PaymentOptionCashOnDelivery : BasePaymentOption
    {
            ...
            HttpResponse webResponse, UserSession userSession)
        {
            try
            {
                webResponse.Redirect(GlobalClass.RootURL + "/Basket/BasketOrderComplete.aspx?InternalPaymentType=cod", false);
            }
            catch (Exception err)
            {
        ...
        {
            return (BaseWebApplication.CashOnDeliveryCurrency);
        }
    }
}
