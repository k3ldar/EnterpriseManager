...
{
    public sealed class PaymentOptionCashOnDelivery : BasePaymentOption
    {
            ...
            HttpResponse webResponse, UserSession userSession)
        {
            try
            {
                webResponse.Redirect(WebsiteSettings.RootURL + "/Shopping/Basket/Order-Complete/Payment-Type/cod/", false);
            }
            catch (Exception err)
            {
        ...
        {
            return (WebsiteSettings.PaymentGateways.CashOnDelivery.Currency);
        }
    }
}
